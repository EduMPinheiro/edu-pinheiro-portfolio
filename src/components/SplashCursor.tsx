...
const splashColors: readonly ColorRGB[] = [
  { r: 0, g: 0.9, b: 1 },
  { r: 0.18, g: 0.36, b: 1 },
  { r: 0.9, g: 0, b: 0.66 }
];

const DEFAULT_BACK_COLOR: ColorRGB = { r: 0, g: 0, b: 0 };
const COLOR_INTENSITY = 0.22;

function canUseSplashCursor() {
...
export default function SplashCursor({
  ...
  const [enabled, setEnabled] = useState(canUseSplashCursor);
  ...
  useEffect(() => {
    ...
    try {
      ...
      canvas.dataset.splashStatus = 'unavailable';
      setEnabled(false);
      return undefined;
    }
    ...
    resizeCanvas();
    initFramebuffers();
    canvas.dataset.splashStatus = 'ready';
    ...
    function startLoop() {
      if (!isActive || document.hidden || animationFrameId !== null) return;
    ...
    function updateColors(dt: number) {
      colorPhase = wrap(colorPhase + dt * config.COLOR_UPDATE_SPEED * 0.35, 0, splashColors.length);
    ...
    function generateColor(): ColorRGB {
      const currentIndex = Math.floor(colorPhase) % splashColors.length;
      const nextIndex = (currentIndex + 1) % splashColors.length;
      const mix = colorPhase - Math.floor(colorPhase);
      const current = splashColors[currentIndex];
      const next = splashColors[nextIndex];
      return {
        r: (current.r + (next.r - current.r) * mix) * COLOR_INTENSITY,
        g: (current.g + (next.g - current.g) * mix) * COLOR_INTENSITY,
        b: (current.b + (next.b - current.b) * mix) * COLOR_INTENSITY
      };
    }
    ...
    function handleVisibilityChange() {
      ...
      if (pointerInitialized) startLoop();
    }
    ...
    return () => {
      ...
      canvas.width = 1;
      canvas.height = 1;
      canvas.dataset.splashStatus = 'disposed';
    };
  }, [
    enabled,
    SIM_RESOLUTION,
  ...
  return <canvas ref={canvasRef} className="splash-cursor-canvas" data-splash-cursor aria-hidden="true" />;
}
